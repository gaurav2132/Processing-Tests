...
public class BarSketch extends Sketch {
	...
	private static Logger log = Logger.getLogger(BarSketch.class.getName());
	public float bottomY, topY;
	protected float buffer;
	protected int count = 5;
	protected ArrayList<Data> list;
	public static String FILENAME = "eventData.csv";
	...
	public BarSketch(Variable data) {
		super(data);
		bottomY = plotY2 - (Utils.SCALE *30);
		topY = plotY1 + (Utils.SCALE *10);
		buffer = (Utils.SCALE *3);
		
		 //Use system font 'Arial' as the header font with 12 point type
		h1 = parent.createFont("Arial", 12, false);
		//Use system font 'Arial' as the label font with 9 point type
		l1 = parent.createFont("Arial", 9, false);
		
		log.info("Initialized");
	}
	
	protected void fetchData(){
		list = new ArrayList<BarSketch.Data>();
		String[] lines = parent.loadStrings(FILENAME);
		for(String s: lines){
			 String[] pieces = PApplet.split(s, ',');
			 list.add(new Data(Integer.parseInt(pieces[1]), pieces[0]));
		}
	}
	...
	/**
	 * Chart Height, Chart Width, Column Red Amount, Column Green Amount, Column Blue Amount, Column Alpha Amount, Label Placement (1 for below columns, 0 for above columns)
	 * @param filename Filename(1 Row CSV Format)
	 * @param yf1 Y Position of chart
	 * @param xf1 X Position of chart
	 * @param yscale
	 * @param xscale
	 * @param r1
	 * @param g1
	...
	void barchart(		String filename,
	   ...
	   //draw the axis lines.
	   parent.line(xf1-3,yf1+2,xf1+xscale,yf1+2);
	   parent.line(xf1-3,yf1+2,xf1-3,yf1-yscale);
	   ...
	   for (int i = 0; i < lines.length; i++) {
	     ...
	     // TODO make this 10%
	     ymax = PApplet.max(Utils.toIntArray(pieces)) + 10;
	     ...
	     for (int i2 = 0; i2 < xcount; i2++) {
	       ...
	       parent.fill (r1,g1,b1,a1);
	        
	       //Draw the columns to scale.
	       parent.quad(xf1, yf1, xf1, yf1-ysclhght, xf1 + xwidth, yf1-ysclhght, xf1 + xwidth, yf1);
	       ...
	       if (labelplacement < 1) {
	       //Above the columns.
	    	   parent.text(pieces[i2], xf1 + (xwidth / 2), yf1 - (ysclhght + 8));
	       } else {
	         //Below the columns.
	    	   parent.text(pieces[i2], xf1 + (xwidth / 2), yf1 + 8);
	       }
